...
import { Subject, SubjectQuestionType } from '../types/quiz';
import { isSupabaseEnabled, shouldSyncOnline, getMaxHistoryRecords } from './systemConfig';
...
export const userPreferencesStorage = {
  save: (preferences: UserPreferences): void => {
    try {
      localStorage.setItem('kids_practice_test_user_preferences', JSON.stringify({
        ...preferences,
        lastUpdated: new Date().toISOString()
      }));
      // Store user info in a dedicated key
      localStorage.setItem('kids_practice_test_user_info', JSON.stringify({
  ...
  load: (): UserPreferences | null => {
    try {
      const stored = localStorage.getItem('kids_practice_test_user_preferences');
      if (!stored) return null;
      const parsed = JSON.parse(stored);
      const userInfo = localStorage.getItem('kids_practice_test_user_info');
      let userId = '';
      let userName = '';
      if (userInfo) {
  ...
  clear: (): void => {
    try {
      localStorage.removeItem('kids_practice_test_user_preferences');
    } catch (error) {
...
export const gameHistoryStorage = {
  save: (result: Omit<GameResult, 'id' | 'completedAt' | 'username'>, online: boolean): GameResult => {
    try {
      ...
      // Limit history to max records from system config
      const maxRecords = getMaxHistoryRecords();
      const trimmedHistory = updatedHistory.slice(0, maxRecords);
      localStorage.setItem('kids_practice_test_game_history', JSON.stringify(trimmedHistory));
      return gameResult;
    } catch (error) {
  ...
  syncPendingRecords: async (supabaseClient: SupabaseClient, tableName: string) => {
    ...
    if (!isSupabaseEnabled()) {
      console.log('[Storage] Supabase is disabled - skipping sync');
      return;
    }

    // Only run if online and sync is enabled
    if (!navigator.onLine || !shouldSyncOnline()) return;
    ...
    localStorage.setItem('kids_practice_test_game_history', JSON.stringify(merged));
  },

  loadAll: (): GameResult[] => {
    try {
      const stored = localStorage.getItem('kids_practice_test_game_history');
      if (!stored) return [];
      const parsed = JSON.parse(stored);
  ...
  clear: (): void => {
    try {
      localStorage.removeItem('kids_practice_test_game_history');
    } catch (error) {
