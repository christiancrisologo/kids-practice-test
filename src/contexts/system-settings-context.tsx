...
export function SystemSettingsProvider({
    ...
    useEffect(() => {
        const loadSettings = () => {
            try {
                const savedSettings = localStorage.getItem('kids-practice-test-system-settings');
                if (savedSettings) {
    ...
    useEffect(() => {
        ...
        try {
            localStorage.setItem('kids-practice-test-system-settings', JSON.stringify(settings));
        } catch (error) {
